...
+ (NSUInteger)dismissAllOverlaysForView:(UIView *)view animated:(BOOL)animated {
    NSArray *overlays = [self allOverlaysForView:view];
    for (DHToast *toastView in overlays) {
...
- (void)show:(BOOL)animated {
    ...
    if (animated) {
        CGRect frame2 = frame;
        frame2.origin.x = parentFrame.size.width;
        self.frame = frame2;
        [UIView animateWithDuration:0.4 delay:0.0 usingSpringWithDamping:0.5 initialSpringVelocity:0.8 options:UIViewAnimationOptionCurveEaseOut animations:^{
            self.frame = frame;
...
- (void)dismiss:(BOOL)animated {
    if (animated) {
        ...
        [UIView animateWithDuration:0.4 delay:0.0 usingSpringWithDamping:0.5 initialSpringVelocity:0.8 options:UIViewAnimationOptionCurveEaseIn animations:^{
            self.frame = frame;
        } completion:^(BOOL finished) {
...
- (void)commonInit {
    ...
    _messageLabel.textColor = MESSAGE_TEXTCOLOR;
    _messageLabel.textAlignment = MESSAGE_TEXTALIGNMENT;
